---
title: "Data 401 Project 1"
author: "Casey, Edward, Shubham"
date: "10/7/2021"
...
q2['prices'] <- q2$psoda2 + q2$pfries2 + q2$pentree2
keeps <- c('wagediff', 'prpblck', 'crmrte', 'state', 'prppov', 'prpncar', 'income',
           'county', 'density', 'hseval')
keeps2 <- c('prices', 'prpblck', 'crmrte', 'state', 'prppov', 'prpncar', 'income',
           'county', 'density', 'hseval')
q2_keep <- q2[keeps]
q2_keep2 <- q2[keeps2]
...
```{r}
## Histograms of each variable in the data set
hist(q2_keep2[,1], main = str_c("Histogram of ", keeps2[1]), xlab=keeps2[1])
...
```{r}
## Scatterplots of each feature vs. difference in starting wage from before and after the increase in minimum wage
for (i in 2:length(keeps2)) {
  dist <- scatter.smooth(q2_keep2[,i],q2_keep2[,1], xlab = keeps2[i], ylab = keeps2[1])
  print(dist)
}
```
...
```{r}
## Creating the causal model
causal_model2 = lm(prices ~ ., data=q2_keep2)
...
## Checking assumptions for the causal model
par(mfcol=c(2,2))
qqPlot(rstudent(causal_model2),pch=16)
hist(rstudent(causal_model2),main="")
residualPlots(causal_model2,type="rstudent",terms=~1,quadratic=FALSE,pch=16)
plot(rstudent(causal_model2),type="o",pch=16)
abline(h=0,lty="dashed")
par(mfcol=c(1,1))
```
...
```{r}
## Creating the causal model
transformed_causal_model2 = lm(1/(prices**4) ~ ., data=q2_keep2)
summary(transformed_causal_model2)
```

```{r}
## Checking assumptions for the causal model
par(mfcol=c(2,2))
qqPlot(rstudent(transformed_causal_model2),pch=16)
hist(rstudent(transformed_causal_model2),main="")
residualPlots(transformed_causal_model2,type="rstudent",terms=~1,quadratic=FALSE,pch=16)
plot(rstudent(transformed_causal_model2),type="o",pch=16)
abline(h=0,lty="dashed")
par(mfcol=c(1,1))
```
